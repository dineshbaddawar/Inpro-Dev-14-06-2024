...
export default class Lookup extends NavigationMixin(LightningElement) {
    ...
    @api secondarySearchTerm = '';
    @api minimalSearchTermLength = 1
        ...
        this._searchResults = resultsLocal.map((result) => {
            ...
            if (this._searchTerm.length > 0) {
                ...
                result.subtitleFormatted = result.subtitle ?
                    ...
                    result.subtitle;
                    result.subtitle2Formatted = result.subtitle2 ?
                    result.subtitle2.replace(regex, '<strong>$1</strong>') :
                    result.subtitle2;
            } else {
                result.titleFormatted = result.title;
                result.subtitleFormatted = result.subtitle;
    ...
    updateSearchTerm(newSearchTerm) {
        ...
        this._searchThrottlingTimeout = setTimeout(() => {
            ...
            if (this._cleanSearchTerm.length >= this.minimalSearchTermLength) {
                ...
                this.loading = true;
                //console.log("secondTerm: " + this.secondarySearchTerm);
